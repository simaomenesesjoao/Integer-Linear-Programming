...
import { Controls as It, MOUSE as G, Vector3 as j, Vector2 as R, Quaternion as qt, MathUtils as Ct, Matrix4 as At, Object3D as Yt } from "three";
import { Graphics as Z, Text as wt, Application as zt } from "pixi.js";
const ut = { type: "change" }, gt = { type: "start" }, yt = { type: "end" }, Mt = 1e-6, v = { NONE: -1, ROTATE: 0, ZOOM: 1, PAN: 2, TOUCH_ROTATE: 3, TOUCH_ZOOM_PAN: 4 }, et = new R(), U = new R(), Xt = new j(), it = new j(), mt = new j(), Y = new qt(), Et = new j(), st = new j(), _t = new j(), ot = new j();
class Gt extends It {
  /**
  ...
  constructor(t, e = null) {
    super(t, e), this.screen = { left: 0, top: 0, width: 0, height: 0 }, this.rotateSpeed = 1, this.zoomSpeed = 1.2, this.panSpeed = 0.3, this.noRotate = !1, this.noZoom = !1, this.noPan = !1, this.staticMoving = !1, this.dynamicDampingFactor = 0.2, this.minDistance = 0, this.maxDistance = 1 / 0, this.minZoom = 0, this.maxZoom = 1 / 0, this.keys = [
      ...
      "KeyD"
      /*D*/
    ], this.mouseButtons = { LEFT: G.ROTATE, MIDDLE: G.DOLLY, RIGHT: G.PAN }, this.target = new j(), this.state = v.NONE, this.keyState = v.NONE, this._lastPosition = new j(), this._lastZoom = 1, this._touchZoomDistanceStart = 0, this._touchZoomDistanceEnd = 0, this._lastAngle = 0, this._eye = new j(), this._movePrev = new R(), this._moveCurr = new R(), this._lastAxis = new j(), this._zoomStart = new R(), this._zoomEnd = new R(), this._panStart = new R(), this._panEnd = new R(), this._pointers = [], this._pointerPositions = {}, this._onPointerMove = $t.bind(this), this._onPointerDown = Kt.bind(this), this._onPointerUp = Qt.bind(this), this._onPointerCancel = Jt.bind(this), this._onContextMenu = re.bind(this), this._onMouseWheel = ne.bind(this), this._onKeyDown = ee.bind(this), this._onKeyUp = te.bind(this), this._onTouchStart = ae.bind(this), this._onTouchMove = he.bind(this), this._onTouchEnd = ce.bind(this), this._onMouseDown = ie.bind(this), this._onMouseMove = se.bind(this), this._onMouseUp = oe.bind(this), this._target0 = this.target.clone(), this._position0 = this.object.position.clone(), this._up0 = this.object.up.clone(), this._zoom0 = this.object.zoom, e !== null && (this.connect(e), this.handleResize()), this.update();
  }
  connect(t) {
  ...
  update() {
    this._eye.subVectors(this.object.position, this.target), this.noRotate || this._rotateCamera(), this.noZoom || this._zoomCamera(), this.noPan || this._panCamera(), this.object.position.addVectors(this.target, this._eye), this.object.isPerspectiveCamera ? (this._checkDistances(), this.object.lookAt(this.target), this._lastPosition.distanceToSquared(this.object.position) > Mt && (this.dispatchEvent(ut), this._lastPosition.copy(this.object.position))) : this.object.isOrthographicCamera ? (this.object.lookAt(this.target), (this._lastPosition.distanceToSquared(this.object.position) > Mt || this._lastZoom !== this.object.zoom) && (this.dispatchEvent(ut), this._lastPosition.copy(this.object.position), this._lastZoom = this.object.zoom)) : console.warn("THREE.TrackballControls: Unsupported camera type.");
  }
  /**
   * Resets the controls to its initial state.
   */
  reset() {
    this.state = v.NONE, this.keyState = v.NONE, this.target.copy(this._target0), this.object.position.copy(this._position0), this.object.up.copy(this._up0), this.object.zoom = this._zoom0, this.object.updateProjectionMatrix(), this._eye.subVectors(this.object.position, this.target), this.object.lookAt(this.target), this.dispatchEvent(ut), this._lastPosition.copy(this.object.position), this._lastZoom = this.object.zoom;
  }
  _panCamera() {
    if (U.copy(this._panEnd).sub(this._panStart), U.lengthSq()) {
      ...
      U.multiplyScalar(this._eye.length() * this.panSpeed), it.copy(this._eye).cross(this.object.up).setLength(U.x), it.add(Xt.copy(this.object.up).setLength(U.y)), this.object.position.add(it), this.target.add(it), this.staticMoving ? this._panStart.copy(this._panEnd) : this._panStart.add(U.subVectors(this._panEnd, this._panStart).multiplyScalar(this.dynamicDampingFactor));
    }
  }
  _rotateCamera() {
    ot.set(this._moveCurr.x - this._movePrev.x, this._moveCurr.y - this._movePrev.y, 0);
    let t = ot.length();
    t ? (this._eye.copy(this.object.position).sub(this.target), Et.copy(this._eye).normalize(), st.copy(this.object.up).normalize(), _t.crossVectors(st, Et).normalize(), st.setLength(this._moveCurr.y - this._movePrev.y), _t.setLength(this._moveCurr.x - this._movePrev.x), ot.copy(st.add(_t)), mt.crossVectors(ot, this._eye).normalize(), t *= this.rotateSpeed, Y.setFromAxisAngle(mt, t), this._eye.applyQuaternion(Y), this.object.up.applyQuaternion(Y), this._lastAxis.copy(mt), this._lastAngle = t) : !this.staticMoving && this._lastAngle && (this._lastAngle *= Math.sqrt(1 - this.dynamicDampingFactor), this._eye.copy(this.object.position).sub(this.target), Y.setFromAxisAngle(this._lastAxis, this._lastAngle), this._eye.applyQuaternion(Y), this.object.up.applyQuaternion(Y)), this._movePrev.copy(this._moveCurr);
  }
  _zoomCamera() {
    let t;
    this.state === v.TOUCH_ZOOM_PAN ? (t = this._touchZoomDistanceStart / this._touchZoomDistanceEnd, this._touchZoomDistanceStart = this._touchZoomDistanceEnd, this.object.isPerspectiveCamera ? this._eye.multiplyScalar(t) : this.object.isOrthographicCamera ? (this.object.zoom = Ct.clamp(this.object.zoom / t, this.minZoom, this.maxZoom), this._lastZoom !== this.object.zoom && this.object.updateProjectionMatrix()) : console.warn("THREE.TrackballControls: Unsupported camera type")) : (t = 1 + (this._zoomEnd.y - this._zoomStart.y) * this.zoomSpeed, t !== 1 && t > 0 && (this.object.isPerspectiveCamera ? this._eye.multiplyScalar(t) : this.object.isOrthographicCamera ? (this.object.zoom = Ct.clamp(this.object.zoom / t, this.minZoom, this.maxZoom), this._lastZoom !== this.object.zoom && this.object.updateProjectionMatrix()) : console.warn("THREE.TrackballControls: Unsupported camera type")), this.staticMoving ? this._zoomStart.copy(this._zoomEnd) : this._zoomStart.y += (this._zoomEnd.y - this._zoomStart.y) * this.dynamicDampingFactor);
  }
...
function oe() {
  this.state = v.NONE, this.dispatchEvent(yt);
}
function ne(s) {
  if (this.enabled !== !1 && this.noZoom !== !0) {
    ...
    this.dispatchEvent(gt), this.dispatchEvent(yt);
  }
}
function re(s) {
...
const X = new j(), Tt = new At(), St = new At(), Pt = new j(), Lt = new j();
class de {
  /**
  ...
  constructor(t = {}) {
    ...
    l.style.overflow = "hidden", this.domElement = l, this.sortObjects = !0, this.getSize = function() {
      ...
      p.matrixWorldAutoUpdate === !0 && p.updateMatrixWorld(), m.parent === null && m.matrixWorldAutoUpdate === !0 && m.updateMatrixWorld(), Tt.copy(m.matrixWorldInverse), St.multiplyMatrices(m.projectionMatrix, Tt), u(p, p, m), this.sortObjects && b(p);
    ...
    function u(p, m, _) {
      ...
      if (p.isCSS2DObject) {
        X.setFromMatrixPosition(p.matrixWorld), X.applyMatrix4(St);
        const g = X.z >= -1 && X.z <= 1 && p.layers.test(_.layers) === !0, y = p.element;
        y.style.display = g === !0 ? "" : "none", g === !0 && (p.onBeforeRender(e, m, _), y.style.transform = "translate(" + -100 * p.center.x + "%," + -100 * p.center.y + "%)translate(" + (X.x * n + n) + "px," + (-X.y * r + r) + "px)", y.parentNode !== l && l.appendChild(y), p.onAfterRender(e, m, _));
        const C = {
    ...
    function d(p, m) {
      return Pt.setFromMatrixPosition(p.matrixWorld), Lt.setFromMatrixPosition(m.matrixWorld), Pt.distanceToSquared(Lt);
    }
    function f(p) {
      const m = [];
...
function Ae(s, t, e, i) {
  const o = document.getElementById(e), n = new de();
  ...
  l.setPixelRatio(window.devicePixelRatio), l.setSize(s, t), a.position.set(0, 0, 15);
  const h = {
    camera_tilt: 0,
    z_height: 0,
    a_coef: 0
  ...
  function d(E, O = {}) {
    ...
    const {
      color: P = 65484,
      opacity: D = 0.7,
      side: at = c.DoubleSide
    } = O, $ = new c.Plane().setFromCoplanarPoints(
      E[0],
      E[1],
      E[2]
    ).normal.clone(), Q = new c.Vector3(), q = new c.Vector3();
    Q.crossVectors(
      Math.abs($.x) > 0.9 ? new c.Vector3(0, 1, 0) : new c.Vector3(1, 0, 0),
      $
    ).normalize(), q.crossVectors($, Q);
    const ct = E.map(
      (tt) => new c.Vector2(tt.dot(Q), tt.dot(q))
    ), lt = c.ShapeUtils.triangulateShape(ct, []), vt = [];
    for (const tt of lt)
      for (const Bt of tt) {
        const pt = E[Bt];
        vt.push(pt.x, pt.y, pt.z);
      }
    const dt = new c.BufferGeometry();
    dt.setAttribute(
      "position",
      new c.Float32BufferAttribute(vt, 3)
    ), dt.computeVertexNormals();
    const Ht = new c.MeshStandardMaterial({
      color: P,
      transparent: D < 1,
      opacity: D,
      side: at
    });
    return new c.Mesh(dt, Ht);
  }
  function f(E, O, P = 16777215) {
    const D = new c.Vector3().subVectors(O, E), at = D.length(), ht = 0.02, $ = new c.CylinderGeometry(
      ht,
      ht,
      at,
      64
    ), Q = new c.MeshStandardMaterial({ color: P }), q = new c.Mesh($, Q), ct = new c.Vector3().addVectors(E, O).multiplyScalar(0.5);
    q.position.copy(ct);
    const lt = new c.Vector3(0, 1, 0);
    return q.quaternion.setFromUnitVectors(
      lt,
      D.clone().normalize()
    ), r.add(q), q;
  }
  function b(E, O) {
    ...
    P.className = "label", P.textContent = E, P.style.color = "white", P.style.fontSize = "20px", P.style.background = "rgba(0,0,0,0.6)", P.style.padding = "4px 8px", P.style.borderRadius = "6px", P.style.border = "1px solid rgba(255,255,255,0.3)";
    const D = new le(P);
    return D.position.copy(O), r.add(D), D;
  }
  function p(E, O) {
    E.position.copy(O);
  }
  let m = b("Start", new c.Vector3(0.3, -0.5, 0));
  f(new c.Vector3(-2, 0, 0), new c.Vector3(10, 0, 0)), f(new c.Vector3(0, -2, 0), new c.Vector3(0, 10, 0)), f(new c.Vector3(-2, 5, 0), new c.Vector3(5, -2, 0)), f(new c.Vector3(-2, 11, 0), new c.Vector3(11, -2, 0)), f(new c.Vector3(0, 0, -2), new c.Vector3(0, 0, 8));
  ...
  r.add(T, C, A, w, M);
  const k = new c.PointLight(16777215, 20);
  k.position.set(2, 2, 5);
  const V = new c.AmbientLight(16777215, 0.1);
  r.add(V), r.add(k), new c.PointLightHelper(k);
  const W = new c.GridHelper(200, 50);
  W.rotation.x = Math.PI / 2;
  function H(E, O, P = {}) {
    const D = d(O, P);
    E.geometry.dispose(), E.geometry = D.geometry, E.material.dispose(), E.material = D.material;
  }
  function N(E, O) {
    H(M, [
      new c.Vector3(9, 0, 0),
  ...
  function S() {
    h.camera_tilt = 0, h.z_height = 0, h.a_coef = 0, a.position.set(0, 0, 15), a.lookAt(0, 0, 0), m.visible = !0, p(m, new c.Vector3(0.3, -0.5, 0)), y.position.set(0, 0, 0), N(0, 0), L = 100;
  }
  let L = 100;
  function B() {
    if (L -= 1, !(L >= 0)) {
      ...
      if (h.z_height <= 7) {
        m.visible = !1, h.z_height += 0.06, h.z_height > 7 && (L = 100), N(h.a_coef, h.z_height);
        return;
      }
      if (h.a_coef <= 1) {
        h.a_coef += 0.01, h.a_coef > 1 && (L = 100), N(h.a_coef, h.z_height);
        return;
      }
      p(m, new c.Vector3(0.3, -0.5, h.a_coef * 3)), y.position.set(0, 0, 3), m.visible = !0, h.a_coef > 1 && (L = 200, S());
    }
  }
  function I() {
    requestAnimationFrame(I), l.render(r, a), n.render(r, a), B(), u.update();
  }
  I();
}
const F = 1e-9;
...
class rt {
  ...
  clone() {
    const t = new rt(structuredClone(this.matrix), this.dimension, this.num_constraints, this.num_artificial);
  ...
  pivot(t, e) {
    pe(this.matrix, t, e);
    const i = this.dictionary[t], o = ft(this.basis, e);
    if (o === void 0)
  ...
  trim_tableau() {
    for (let r = 0; r < this.num_constraints; r++) {
      ...
      if (nt(a, this.basis_dimension - this.num_artificial, this.num_artificial)) {
        let l = !1;
        for (let h = 0; h < this.basis_dimension + this.num_constraints; h++) {
          const u = this.basis[h];
          if (nt(u, this.basis_dimension - this.num_artificial, this.num_artificial))
            continue;
...
function Dt(s, t, e) {
  ...
  let u = 0, d = 0;
  for (const f of s) {
    const b = f.lower, p = f.row, m = f.upper;
    if (b !== void 0) {
      for (let _ = 0; _ < i; _++)
        h[u][_] = -p[_];
...
function be(s, t, e, i) {
  ...
  else if (Math.abs(t) < z)
    l = new x(-e / s, r), h = new x(-e / s, a);
  else {
    const u = -(e + t * r) / s, d = -(e + t * a) / s, f = -(e + s * o) / t, b = -(e + s * n) / t;
    u >= o && u <= n ? l = new x(u, r) : u < o ? l = new x(o, f) : l = new x(n, b), d >= o && d <= n ? h = new x(d, a) : d < o ? h = new x(o, f) : h = new x(n, b);
  }
  return { p1: l, p2: h };
}
class jt {
  ...
  to_window(t) {
    return new K(t.x - this.origin.x, this.origin.y - t.y);
  }
  to_page(t) {
    return new xt(t.x + this.origin.x, this.origin.y - t.y);
  }
}
class Nt {
  windowBotLeft;
  ...
  to_window(t) {
    ...
    return new K(this.windowBotLeft.x + e * this.w_width, this.windowBotLeft.y + i * this.w_height);
  }
}
class Rt {
  pageToWindow;
...
class Ft extends Z {
  ...
  constructor(t, e, i, o) {
    ...
    let n = t.x, r = e.x, a = t.y, l = e.y, h = Math.trunc(n / i), u = Math.trunc(r / i), d = Math.trunc(a / i), f = Math.trunc(l / i), b = Math.abs(Math.round(n / i) - n / i) < z, p = Math.abs(Math.round(r / i) - r / i) < z, m = Math.abs(Math.round(a / i) - a / i) < z, _ = Math.abs(Math.round(l / i) - l / i) < z;
    for (let g = d; g <= f; g++) {
      if (g == d && m || g == f && _)
        continue;
      let y = g * this.delta;
...
function Zt(s, t, e, i = 8, o = Math.PI / 6) {
...
class xe extends Z {
  ...
  draw(t) {
    ...
    this.clear(), this.moveTo(i.x, i.y).lineTo(o.x, o.y), Zt(this, o, i), this.stroke({
      width: 2,
      color: 16711680
    });
  }
...
class ve extends Z {
  ...
  draw(t, e) {
    ...
    a = a.scale(1 / l);
    const h = t, u = new x(i, r), d = new x(o, r), f = new x(o, n), b = new x(i, n), p = [
      J(h, a, this.p1, this.p2),
      J(h, a, u, d),
      J(h, a, d, f),
      J(h, a, f, b),
      J(h, a, b, u)
    ], m = p.reduce((T, w, A) => w.t < 0 ? T : T === -1 || p[T].t < 0 || w.t < p[T].t ? A : T, -1), _ = this.converter.to_page(t), g = this.converter.to_page(p[m].p);
    this.clear(), this.moveTo(_.x, _.y).lineTo(g.x, g.y), m === 0 && Zt(this, g, _), this.stroke({
      width: 1,
      color: 3355443
    });
...
class Ee {
  ...
  update(t) {
    ...
    for (let h = 0; h < r; h++) {
      ...
      for (const _ of i) {
        ...
        if (Math.abs(y) < z) continue;
        const C = y > 0 ? "+" : "-", M = Math.abs(y), T = Math.abs(M - 1) < z ? "" : M.toFixed(1);
        m.append(` ${C} ${T}`);
        const w = document.createElement("span");
        w.className = "highlight", w.textContent = g, m.append(w);
      }
  ...
  updateCostFunction(t) {
    ...
    l.className = "Trow", l.append(`Cost = ${a}`);
    for (const h of i) {
      const u = this.idToLabel.get(h), d = e[n][h];
      if (Math.abs(d) < z) continue;
      const f = d > 0 ? "+" : "-", b = Math.abs(d), p = Math.abs(b - 1) < z ? "" : b.toFixed(1);
      l.append(` ${f} ${p}`);
...
class Wt {
  ...
  addLine(t) {
    ...
    const e = Te(t), i = t.label;
    if (i === "x") {
      if (this.xExists)
        throw "x already exists";
      if (Math.abs(e.a + 1) > z || Math.abs(e.b) > z || Math.abs(e.d) > z)
        throw "Line equation for x label does not match";
      this.xExists = !0;
    } else if (i === "y") {
      if (this.yExists)
        throw "y already exists";
      if (Math.abs(e.b + 1) > z || Math.abs(e.a) > z || Math.abs(e.d) > z)
        throw "Line equation for y label does not match";
      this.yExists = !0;
    }
    const o = new Ce(this.coordLimits, e, this.pageToCoord, i), n = new ve(e, o.p1, o.p2, this.coordLimits, i, this.pageToCoord);
  ...
  getManager(t) {
    ...
    const i = Dt(e, t, !1);
    return new Me(i, this.labelDictionary);
  }
}
function Ut(s, t) {
  const e = [];
  ...
  e.push(new Vt({ lower: void 0, a: 0, b: 1, upper: t.ymax, label: "ytop" })), e.push(new Vt({ lower: void 0, a: 1, b: 0, upper: t.xmax, label: "xtop" }));
  const i = [];
  i.push(0), i.push(0);
  let o = Dt(e, i, !0);
  _e(o), fe(o);
  ...
  draw(t, e = {}) {
    ...
    for (let h = 0; h < l; h++) {
      ...
      for (let d = 0; d < t[h].length; d++) {
        ...
        f.textContent = String(t[h][d]), u.appendChild(f);
      }
      a.appendChild(u);
    }
...
class ze {
  ...
  constructor({ container: t, width: e, height: i, app_width: o, constraints: n, costFunction: r, showMatrix: a, showCoords: l, topRightCoord: h, botLeftCoord: u }) {
    this.container = t, this.width = e, this.height = i, this.app_width = o, this.constraints = n, this.costFunction = r, this.showMatrix = !!a, this.showCoords = !!l, this.topRightCoord = h, this.botLeftCoord = u, this.app = new zt(), this.matrixContainer = a || t, this.coordinatesContainer = l || t;
  }
  async init() {
    await this.app.init({
      width: this.width,
    ...
    const t = new xt(0, this.app_width), e = new jt(t), i = new K(0, 0), o = new K(this.app_width, this.app_width), n = new x(this.topRightCoord.x, this.topRightCoord.y), r = new x(this.botLeftCoord.x, this.botLeftCoord.y), a = { xmin: r.x, xmax: n.x, ymin: r.y, ymax: n.y }, l = (a.xmin + a.xmax) * 0.5, h = (a.ymin + a.ymax) * 0.5, u = new Nt(i, o, r, n), d = new Rt(u, e), f = e.to_page(i), b = e.to_page(o), p = new Z().rect(f.x, b.y, b.x - f.x, f.y - b.y).fill({
      color: 15658734,
      alpha: 0.8
    });
    this.app.stage.addChild(p);
    let m = new Ft(r, n, 1, d);
    this.app.stage.addChild(m);
    const _ = this.constraints, y = Ut(_, a).flatMap((S) => {
      const L = d.to_page(S);
      return [L.x, L.y];
    }), C = new Z();
    ...
    for (const S of _)
      M.addLine(S);
    let T = new x(0, 0);
    this.costFunction !== void 0 && (T = new x(this.costFunction[0], this.costFunction[1]));
    const w = new xe(T, d);
    this.app.stage.addChild(w);
    const A = M.getLines(), k = M.getArrows(), V = M.getManager(this.costFunction), W = new Ee(this.coordinatesContainer, V.tableau.num_constraints, V.idToLabel);
    W.setVisible(this.showCoords);
    const H = new Pe(this.matrixContainer);
    this.showMatrix && H.draw(V.tableau.matrix, { fontSize: 20 });
    for (const S of A)
      this.app.stage.addChild(S.visual), this.app.stage.addChild(S.labelText);
    for (const S of A)
      this.app.stage.addChild(S.hit);
    this.app.stage.addChild(...k);
    for (const S of k)
      this.app.stage.addChild(S.lab);
    let N = d.to_page(new x(l, h));
    for (const S of A)
      S.onClick = () => {
        console.log("Clicked"), V.update(S.id, S.active);
      }, S.onMouseMove = (L) => {
        const B = L.global;
        N.x = B.x, N.y = B.y;
        const I = d.to_coord(N);
        for (const E of k)
          E.update(I);
        w.update(I);
      };
    V.onUpdate = () => {
      ...
      const S = d.to_coord(N);
      for (const L of V.activeLines())
        A[L].activate(), k[L].activate(), k[L].update(S);
      w.update(S);
      for (const L of V.inactiveLines())
        A[L].deactivate(), k[L].deactivate();
      this.showCoords && (W.update(V.tableau), W.resetWarning()), this.showMatrix && H.draw(V.tableau.matrix, { fontSize: 20 });
    }, V.onWarning = () => {
      this.showCoords && W.setWarning();
    }, V.onUpdate(), p.eventMode = "static", p.on("pointermove", (S) => {
      const L = S.global;
      N.x = L.x, N.y = L.y;
      const B = d.to_coord(N);
      for (const I of k)
        I.update(B);
      w.update(B);
    });
...
class ke {
  ...
  constructor({ container: t, width: e, height: i, app_width: o, constraints: n, include_polygon: r, interactive_lines: a, active_lines: l, topRightCoord: h, botLeftCoord: u }) {
    this.container = t, this.width = e, this.height = i, this.app_width = o, this.constraints = n, this.include_polygon = r, this.interactive_lines = a, this.active_lines = l, this.topRightCoord = h, this.botLeftCoord = u, this.app = new zt();
  }
  async init() {
    await this.app.init({
      width: this.width,
    ...
    this.app.stage.addChild(p);
    const m = Ut(this.constraints, a), _ = new Se(m, d);
    this.include_polygon && this.app.stage.addChild(_);
    let g = new Ft(r, n, 1, d);
    this.app.stage.addChild(g);
    const y = new Wt(a, d);
    for (const w of this.constraints)
      y.addLine(w);
    const C = y.getLines(), M = y.getArrows();
    for (const w of C)
      this.app.stage.addChild(w.visual);
    for (const w of C)
      this.app.stage.addChild(w.hit);
    this.app.stage.addChild(...M);
    for (const w of M)
      this.app.stage.addChild(w.lab);
    for (let w = 0; w < C.length; w++)
      this.interactive_lines.includes(w) && (C[w].onClick = () => {
        M[w].toggleActivate(), C[w].toggleActivate();
      });
    for (let w = 0; w < C.length; w++)
      C[w].onMouseMove = (A) => {
        const k = A.global;
        T.x = k.x, T.y = k.y;
        const V = d.to_coord(T);
        for (const W of M)
          W.update(V);
      };
    for (let w = 0; w < C.length; w++)
      this.active_lines.includes(w) && (M[w].activate(), C[w].activate());
    let T = d.to_page(new x(l, h));
    p.eventMode = "static", p.on("pointermove", (w) => {
      const A = w.global;
      T.x = A.x, T.y = A.y;
      const k = d.to_coord(T);
      for (const V of M)
        V.update(k);
    }), this.include_polygon && (_.onMove = (w) => {
      const A = w.global;
      T.x = A.x, T.y = A.y;
      const k = d.to_coord(T);
      for (const V of M)
