...
Tableau build_tableau(const Constraints& constraints, const VectorD& cost){
    ...
    Matrix matrix = Matrix::Zero(num_constraints+2, num_dimensions+num_constraints+num_artificial+1);
...
bool simplex(Tableau& tableau){
    ...
    while(true){
        ...
        bool pivoted = tableau.simplex_pivot(*index_largest);
...
std::optional<Solution> solve_lp(const Constraints& constraints, const VectorD& cost){
    ...
    bool successful = simplex(tableau);
    if(!successful){
        return std::nullopt;
    }

    // Feasible region not found
    if(std::abs(tableau.get_cost()) > tol){
        return std::nullopt;
    }

    Tableau tableau2 = tableau.trim_tableau();
    bool successful2 = simplex(tableau2);
    if(!successful2){
        return std::nullopt;
    }

    return Solution{tableau2.get_cartesian_coordinates(), tableau2.get_cost()};
}

VectorD Tableau::get_cartesian_coordinates(){
    VectorD coordinates = VectorD::Zero(basis_dimension);
    
    for(const unsigned int basis_var: basis){
        if(basis_var < basis_dimension){
            coordinates[basis_var] = 0.0;
        }
    }

    for(unsigned int row=0; row<num_constraints; row++){
        unsigned int slack_var = dictionary[row];
        if(slack_var < basis_dimension){
            coordinates[slack_var] = get_rhs()[row];
        }
    }
